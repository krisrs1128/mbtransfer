...
vis_taxa2 <- focus_taxa[seq(1, length(focus_taxa), by = 9)] # 9 works well
...
p1 <- pivot_ts(ts) |>
  ...
  ggplot() +
  geom_point(aes(pH, value), alpha = 0.4, size = 0.3) +
  labs(x = "pH", y = "asinh(Abundance)", title = "(a) pH vs. Abundance") +
  facet_wrap(~ taxon, scales = "free_y", nrow = 1) +
  theme(plot.margin = unit(c(0, 0, 0, 0), "null"))

p2 <- sim_ts |>
  filter(taxon %in% vis_taxa2, time > 35, time < 55) |>
  mutate(
    pH = factor(round(plateaus[as.integer(counterfactual)], 3)),
    taxon = factor(taxon, levels = taxa(ts)[taxa_order])
    ) |>
...
ggsave("aqua-combined.png", width = 12, height = 6)
```