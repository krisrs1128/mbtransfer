...
plot_cp_profile <- function(cp_profile, max_value = 8.5) {
  p1 <- cp_profile |>
  ...
  p2 <- cp_profile |>
    ...
    group_by(g1 = cut(taxon1_lag1, 100), g2 = cut(taxon2_lag1, 20)) |>
    mutate(
    ...
    ggplot() +
      ...
      scale_fill_scico(palette = "glasgow", midpoint = 12) +
      scale_color_scico(palette = "glasgow", midpoint = 12) +
      scale_x_continuous(expand = c(0, 0)) +
      scale_y_continuous(limits = c(0, max_value), expand = c(0, 0))

  p3 <- cp_profile |>
    filter(`_vname_` %in% c("taxon1_lag1", "taxon2_lag1")) |>
    group_by(g1 = cut(taxon1_lag1, 20, label = F), g2 = cut(taxon2_lag1, 100, label = F)) |>
    summarise(
      y_hat_25 = quantile(`_yhat_`, 0.25),
      y_hat_median = quantile(`_yhat_`, 0.50),
      y_hat_75 = quantile(`_yhat_`, 0.75)
...
ts <- simulate_oscillator(perturb = TRUE) |>
  to_ts()
fit <- mbtransfer(ts[, 1:35], P = 1, Q = 1, alpha = 1e-4)
...
```{r}
patches <- mbtransfer:::patchify_df(ts, 1, 1)
ix <- sample(1:nrow(patches$x), 5000)
...
cp_profile <- predict_profile(explainer, patches$x[ix, ])
tax2 <- plot_cp_profile(cp_profile, 20)
```
