...
simulate_oscillator <- function(n_time = 10, n_series = 1000, perturb = FALSE) {
...
fit <- mbtransfer(ts[, 1:35], P = 3, Q = 1, alpha = 0, lambda = 0) # 1e-6)
ts_missing <- subset_values(ts, 1:35)
ts_preds <- predict(fit, ts_missing[1:8])
...
fit <- mbtransfer(ts[, 1:35], P = 3, Q = 1, alpha = 0, lambda = 0)
ts_missing <- subset_values(ts, 1:35)
ts_preds <- predict(fit, ts_missing[1:8])
plot_ts(ts[1:8], ts_preds) +
  geom_vline(xintercept = 35) +
  geom_rect(xmin = 15, xmax = 20, ymin = -Inf, ymax = Inf, alpha = 0.01, fill = "#f6f6f6") +
  geom_rect(xmin = 40, xmax = 45, ymin = -Inf, ymax = Inf, alpha = 0.01, fill = "#f6f6f6")
```
