...
correlations across panels. Overall, the out-of-sample correlation between the
truth and predictions is a little over $\hat{\rho} = 0.57$. This can vary quite
significantly on the lag and quantile of abundance, though, ranging from
$\hat{\rho} = 0.123$ to 0.558.
...
reshaped_preds |>
  ...
  write_csv("~/Downloads/diet-correlations.csv")

reshaped_preds |>
  filter(generalization == "out-of-sample") |>
  select(y, y_hat) |>
  summarise(overall = cor(y, y_hat))
```
