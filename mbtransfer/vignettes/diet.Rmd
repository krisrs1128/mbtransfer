...
`Citep(bib, "David2013DietRA")` was one of the first studies to draw attention
...
ts_preds[["in-sample"]] <- predict(fit[["in-sample"]], ts_missing)
```
...
```{r}
fits[["out-of-sample"]] <- mbtransfer(ts[c(1:5, 11:15)], P = 3, Q = 3)
...
values_df |>
  ...
  group_by(taxon) |>
  mutate(value = rank(value) / n()) |>
  interaction_hm(taxa = c("Otu000011", "Otu000017", "Otu000142"), "diet")
```